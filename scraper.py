...
for ani in ani_list:
  ...
  if 'tid' in ani:
    ...
    premiered = soup.find('table', class_='data').find_all('tr')[2].select_one('td').get_text().split('～')[0].split('-') # 무조건 다시 생성
    ani['premiered'] = datetime.datetime.strptime(premiered[0] + '-' + premiered[1].zfill(2), '%Y-%m')
